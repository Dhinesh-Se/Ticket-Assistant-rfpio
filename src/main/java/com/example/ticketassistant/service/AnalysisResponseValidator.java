...
public class AnalysisResponseValidator {
    private static final Set<String> REQUIRED = Set.of("category", "summary", "suggestedResponse", "recommendedTeam",
            "confidence");
    ...
    public ValidatedAnalysis validate(String rawResponse) {
        try {
            ...
            if (!root.isObject() || root.size() != REQUIRED.size())
                throw new InvalidAnalysisException("Analysis must contain exactly the required fields");
            ...
            if (!Double.isFinite(confidence) || confidence < 0 || confidence > 1)
                throw new InvalidAnalysisException("Confidence must be between 0 and 1");
            return new ValidatedAnalysis(category, summary, suggestedResponse, team, confidence);
        } catch (InvalidAnalysisException e) {
            throw e;
        } catch (Exception e) {
            throw new InvalidAnalysisException("Analysis is not valid JSON", e);
        }
    }
