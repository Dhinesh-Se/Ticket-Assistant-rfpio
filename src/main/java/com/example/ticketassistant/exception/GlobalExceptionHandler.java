...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ErrorResponse> notFound(TicketNotFoundException e) {
        return error(HttpStatus.NOT_FOUND, "TICKET_NOT_FOUND", "The requested ticket does not exist.");
    }

    @ExceptionHandler({ MethodArgumentNotValidException.class,
            org.springframework.http.converter.HttpMessageNotReadableException.class })
    ResponseEntity<ErrorResponse> invalidRequest(Exception e) {
        return error(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", "Request fields are missing or invalid.");
    }
